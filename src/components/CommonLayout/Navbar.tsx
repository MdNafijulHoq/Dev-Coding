            ...
            <NavigationMenuList className="gap-4">
              {navigationLinks.map((link, index) => (
                <NavigationMenuItem key={index} className="relative">
                  {link.submenu ? (
                    <>
                      <NavigationMenuTrigger className="px-2 py-1 font-medium hover:text-purple-600 cursor-pointer">
                        {link.label}
                      </NavigationMenuTrigger>

                      {/* Submenu positioned below the label */}
                      <NavigationMenuContent className="absolute left-0 top-full mt-2 bg-white shadow-lg border rounded-md p-2 min-w-40 z-50 cursor-pointer">
